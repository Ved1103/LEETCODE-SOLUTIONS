...
public:
    ...
    string countOfAtoms(string formula) {
        stack<unordered_map<string, int>>
            indicesStack; // Stack to store maps of atoms and their counts

        unordered_map<string, int>
            currentAtoms; // Map to store current atom counts
        ...
        while (i < formula.length()) {
            if (isupper(formula[i])) {
                ...
                count =
                    max(1, count); // If no digit follows, count defaults to 1
                currentAtoms[atom] += count;
            } else if (formula[i] == '(') {
                indicesStack.push(
                    currentAtoms);    // Save current atom counts to stack
